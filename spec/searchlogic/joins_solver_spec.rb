...
describe Searchlogic::JoinsSolver do
  describe '#solve!' do
    ...
    context 'current_hash' do
      ...
      it { should eql({:current => :value}) }
    end

    context 'new_hash' do
      subject { new_hash }
      it { should eql({:new => :value}) }
    end
  end
  ...
  context "#solve!" do
    it "properly merges duplicate joins" do
      ...
      new_find_options.should eql({:joins => [company_join]})
    end
    ...
    context "comparing" do
      ...
      it "doesnt care about condition order" do
        ...
        Searchlogic::JoinsSolver.new(User, current_find_options, new_find_options).solve!
        new_find_options.should eql({:joins => [company_join_with_extra_condition]})
      end
      ...
      it "doesnt care if multiple joins are in a single string" do
        ...
        Searchlogic::JoinsSolver.new(User, current_find_options, new_find_options).solve!
        new_find_options.should eql({:joins => [company_join_with_extra_condition, company_join_with_alias]})
      end
      ...
      it "doesn't care about has many through symbol joins" do
        ...
        current_find_options.should eql({:joins => joins})
      end
      ...
      it "doesnt care about join type and gives inner joins priority" do
        ...
        new_find_options = {:joins => company_join}
        Searchlogic::JoinsSolver.new(User, current_find_options, new_find_options).solve!
        current_find_options.should eql({:joins => [company_join]})
      end
    end
    ...
    context 'aliasing' do
      it "aliases different joins with the same name" do
        ...
        Searchlogic::JoinsSolver.new(User, current_find_options, new_find_options).solve!
        new_find_options.should eql({:joins => ["INNER JOIN companies companies_1 ON companies_1.id = users.company_id AND companies_1.name = 'Concierge Live'"]})
      end
      ...
      it "uses aliases already established" do
        ...
        new_find_options = {:joins => company_join}
        Searchlogic::JoinsSolver.new(User, current_find_options, new_find_options).solve!
        new_find_options.should eql({:joins => [company_join_with_alias]})
      end
      ...
      it "carries over to the group" do
        ...
        Searchlogic::JoinsSolver.new(User, current_find_options, new_find_options).solve!
        new_find_options.should eql({:joins => [company_join_with_alias], :group => "aliased_companies.name"})
      end
      ...
      it "carries over to the dependent joins" do
        ...
        Searchlogic::JoinsSolver.new(User, current_find_options, new_find_options).solve!
        new_find_options.should eql({:joins => [company_join_with_alias, "INNER JOIN departments ON departments.company_id = aliased_companies.id"]})
      end
      ...
      it "carries over to the select" do
        ...
        Searchlogic::JoinsSolver.new(User, current_find_options, new_find_options).solve!
        new_find_options.should eql({:joins => [company_join_with_alias], :select => "aliased_companies.name"})
      end
      ...
      it "carries over to the conditions" do
        ...
        Searchlogic::JoinsSolver.new(User, current_find_options, new_find_options).solve!
        new_find_options.should eql({:joins => [company_join_with_alias], :conditions => "aliased_companies.name = 'Concierge Live'"})
      end
      ...
      it "carries over to the order" do
        ...
        Searchlogic::JoinsSolver.new(User, current_find_options, new_find_options).solve!
        new_find_options.should eql({:joins => [company_join_with_alias], :order => "aliased_companies.name ASC"})
      end
